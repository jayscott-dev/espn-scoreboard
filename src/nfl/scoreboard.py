...
from nfl.game import NFLGame

import utils.date as date_utils

@dataclass
class NFLScoreboard(Scoreboard):
    _games: list[NFLGame]

    @classmethod
    def from_dict(cls, data: dict) -> NFLScoreboard:
        games = [NFLGame.from_dict(event) for event in data.get("events", [])]
